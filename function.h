...
typedef struct
{
    ...
    // Lưu thời điểm gần nhất thiết bị phản hồi để phục vụ logic nhóm
    unsigned long lastResponseMillis[MAX_DEVICES];
    // Đánh dấu thiết bị còn đang chờ phản hồi trong chu kỳ hiện tại
    bool pendingResponse[MAX_DEVICES];
    int deviceCount;
} device_info;
